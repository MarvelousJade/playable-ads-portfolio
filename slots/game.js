...
(function () {
  ...
  var W = 720, H = 1280;
  // SwiftShader CI runners can render particle-heavy Phaser frames far slower
  // than wall time. Test mode preserves the funnel while reducing visual load.
  ...
  var GameScene = new Phaser.Class({
    ...
    buildCoinEmitter: function () {
      this.coins = this.add.particles(0, -60, 'coin', {
        ...
        rotate: { min: 0, max: 360 }, quantity: TEST_MODE ? 1 : 3, frequency: TEST_MODE ? 120 : 45, emitting: false
      });
    ...
    buildSpinButton: function () {
      ...
      // The persistent install pill (bottom-right) is excluded.
      this.input.on('pointerdown', function (pointer) {
        if (pointer.y > 240 && !(this.pillBounds && this.pillBounds.contains(pointer.x, pointer.y))) {
          this.onSpinPressed();
        }
      }, this);
    ...
    buildInstallPill: function () {
      var px = W - 24 - 150, py = H - 24 - 54;
      this.pillBounds = new Phaser.Geom.Rectangle(px, py, 150, 54);
      var pill = this.add.container(px + 75, py + 27).setDepth(90);
      var g = this.add.graphics();
      g.fillStyle(0x1b8f3a, 1); g.fillRoundedRect(-75, -27, 150, 54, 27);
      g.lineStyle(3, 0xffe27a, 1); g.strokeRoundedRect(-75, -27, 150, 54, 27);
      var t = this.add.text(0, 0, 'INSTALL', {
    ...
    onSpinPressed: function () {
      if (this.spinning || this.chestOpen || this.freeSpins <= 0) return;
    ...
    onAllStopped: function (outcome) {
      ...
      if (outcome.jackpot) {
        ...
        // after the first (teaser) win, offer the pick-a-chest bonus round —
        // gives the player a real choice before the jackpot spin
        if (this.scriptIndex === 1) {
          this.chestOpen = true;   // lock spins now; overlay appears after the win beat
          this.time.delayedCall(1300, this.showChestBonus, [], this);
        } else {
          this.enableSpin();
        }
      }
    ...
    // ── Pick-a-chest bonus round (player agency between spins) ────────────────
    showChestBonus: function () {
      var self = this;
      PlayableAd.track('bonus_offered');
      var ov = this.add.container(0, 0).setDepth(70);
      var dim = this.add.rectangle(0, 0, W, H, 0x05030c, 0.8).setOrigin(0).setInteractive();
      ov.add(dim);
      var title = this.add.text(W / 2, 400, 'BONUS!', {
        fontFamily: 'Arial Black, Arial', fontSize: '72px', color: '#ffd23c'
      }).setOrigin(0.5).setShadow(0, 4, '#7a1010', 6);
      var sub = this.add.text(W / 2, 470, 'Pick a chest', {
        fontFamily: 'Arial', fontSize: '34px', color: '#ffffff'
      }).setOrigin(0.5);
      ov.add([title, sub]);
      [200, 360, 520].forEach(function (x, i) {
        var chest = self.add.text(x, 660, '🎁', {
          fontFamily: '"Segoe UI Emoji", Arial', fontSize: '96px'
        }).setOrigin(0.5);
        self.tweens.add({ targets: chest, y: 636, duration: 480 + i * 130, yoyo: true, repeat: -1, ease: 'Sine.inOut' });
        // generous finger-sized tap zone (emoji text bounds are unreliable targets)
        var zone = self.add.zone(x, 648, 160, 170).setInteractive();
        zone.once('pointerdown', function () { self.pickChest(ov, chest); });
        ov.add([chest, zone]);
      });
    },

    pickChest: function (ov, chest) {
      if (this.chestPicked) return;   // one pick only — other chests keep bobbing
      this.chestPicked = true;
      var self = this;
      var BONUS = 2500;
      PlayableAd.track('bonus_pick');
      this.tweens.add({ targets: chest, scale: 1.5, angle: { from: -8, to: 8 }, duration: 160, yoyo: true, repeat: 1 });
      chest.setText('💰');
      this.coins.explode(TEST_MODE ? 2 : 24, chest.x, chest.y);
      SFX.win();
      this.addWinning(BONUS);
      this.flashText('+' + BONUS.toLocaleString() + ' BONUS!', '#ffd23c');
    ...
    addWinning: function (amount) {
      var self = this;
      this.coins.explode(TEST_MODE ? 3 : 40, W / 2, WIN_TOP + WIN_H / 2);
      var from = balance;
      balance += amount;
      var o = { v: from };
      this.tweens.add({
    ...
    showEndCard: function () {
      ...
      var sub = this.add.text(W / 2, 450, 'Claim your ' + balance.toLocaleString() + ' coins!', {
        fontFamily: 'Arial', fontSize: '34px', color: '#ffffff'
      }).setOrigin(0.5);
      ...
      var ct = this.add.text(0, -4, 'CLAIM NOW', {
